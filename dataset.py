...
from graph4nlp.pytorch.modules.graph_construction.base import (
    DynamicGraphConstructionBase, StaticGraphConstructionBase)
...
class Dataset(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # read data, parse it, build graphs for it, build vocab from it, vectorize it and save data.pt and vocab.pt
        self._process()
        ...
        if self.for_inference:
            ...
            if hasattr(self, "reused_label_model"):
                self.label_model = LabelModel.build(
                    self.processed_file_paths["label"])
    ...
    def read_raw_data(self):
        ...
        if "val" in self.raw_file_paths.keys():
            ...
            if self.val_split_ratio > 0:
                new_train_length = int(
                    (1 - self.val_split_ratio) * len(self.train))
                import random
    ...
    def _build_topology_process(
        ...
        if static_or_dynamic not in ["static", "dynamic"]:
            raise ValueError(
                "Argument: ``static_or_dynamic`` must be ``static`` or ``dynamic``")
        ret = []
        if static_or_dynamic == "static":
            ...
            if topology_builder == IEBasedGraphConstruction:
                ...
                raise NotImplementedError(
                    "unknown static graph type: {}".format(graph_name))
            print("CoreNLP server connected.")
            pop_idxs = []
            for cnt, item in enumerate(data_items):
                if cnt % 1000 == 0:
                    print("Port {}, processing: {} / {}".format(port,
                          cnt, len(data_items)))
                try:
                ...
                ret.append(item)
            # validation on each element to be not has exxception
            ret = [x for idx, x in enumerate(ret) if idx not in pop_idxs]
        elif static_or_dynamic == "dynamic":
            # build graph with no edge for each item, which in each work will be token for each node in graph
            if graph_name == "node_emb":
                ...
                raise RuntimeError(
                    "Unknown dynamic_graph_type: {}".format(graph_name))

        else:
            raise NotImplementedError(
                "Currently only static and dynamic are supported!")
    ...
    def _process(self):
        ...
        if self.for_inference:
            ...
            # extract all tokens and convert them to vectors according to pretrained_word_emb_name
            self.build_vocab()
...
class Text2LabelDataset(Dataset):
        ...
        **kwargs,
        ...
        if graph_name == "dependency":
            ...
            if topology_builder is None:
                raise ValueError(
                    "``topology_builder`` can't be None if graph is defined by user.")
            if static_or_dynamic is None:
                raise ValueError(
                    "``static_or_dynamic`` can't be None if graph is defined by user.")
    ...
    def build_vocab(self):
        ...
        if "val" in self.__dict__:
            all_labels = all_labels.union(
                {item.output_label for item in self.val})
    ...
    def _vectorize_one_dataitem(cls, data_item, vocab_model, label_model=None, use_ie=False):
        ...
        for node_idx in range(graph.get_node_num()):
            ...
            node_token_id = vocab_model.in_word_vocab.getIndex(
                node_token, use_ie)
        ...
        if use_ie and "token" in graph.edge_attributes[0].keys():
            ...
            for edge_idx in range(graph.get_edge_num()):
                ...
                edge_token_id = vocab_model.in_word_vocab.getIndex(
                    edge_token, use_ie)
            ...
            if use_ie:
                ...
                edge_token_matrix = torch.tensor(
                    edge_token_matrix, dtype=torch.long)
